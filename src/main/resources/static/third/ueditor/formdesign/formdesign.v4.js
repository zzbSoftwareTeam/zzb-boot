...
UE.plugins['checkbox'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        evt = evt || window.event;
        var el = evt.target || evt.srcElement;
        var plugins = el.getAttribute('plugins');
        if ( /input/ig.test( el.tagName ) && plugins==thePlugins) {
            var html = popup.formatHtml(
    ...
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
                ...
                editor:this,
                title: '下拉菜单',
                cssRules:"width:590px;height:420px;",
                buttons:[
                {
                    className:'edui-okbutton',
                    label:'确定',
 ...
 * @example
 * ```javascript
 * editor.execCommand( 'enumlink');
 * ```
 */
UE.plugins['select-ext'] = function () {
    var me = this,thePlugins = 'select-ext';
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
                iframeUrl:this.options.UEDITOR_HOME_URL + UE.formDesignUrl+'/select-ext.html',
                name:thePlugins,
                editor:this,
                title: '下拉菜单-扩展',
                cssRules:"width:590px;height:435px;",
                buttons:[
                {
                    className:'edui-okbutton',
                    label:'确定',
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        if ( /select|span/ig.test( el.tagName ) && leipiPlugins==thePlugins) {
            var html = popup.formatHtml(
                '<nobr>下拉菜单-扩展: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
            if ( html ) {
                if(el.tagName=='SPAN')
                {
                    var elInput = el.getElementsByTagName("select-ext");
                    el = elInput.length>0 ? elInput[0] : el;
                }
                popup.getDom( 'content' ).innerHTML = html;
                popup.anchorEl = el;
...
UE.plugins['button'] = function () {
	...
	me.commands[thePlugins] = {
			execCommand:function () {
				var dialog = new UE.ui.Dialog({
					iframeUrl:this.options.UEDITOR_HOME_URL + UE.formDesignUrl+'/button.html',
					name:thePlugins,
					editor:this,
					title: '按钮',
					cssRules:"width:590px;height:220px;",
					buttons:[
					         {
					        	 className:'edui-okbutton',
					        	 label:'确定',
	...
	me.addListener( 'mouseover', function( t, evt ) {
		...
		var el = evt.target || evt.srcElement;
		var leipiPlugins = el.getAttribute('plugins');
		if ( /a/ig.test( el.tagName ) && leipiPlugins==thePlugins) {
			var html = popup.formatHtml(
